...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-bold text-2xl text-gray-800 dark:text-gray-200 leading-tight">Authors Management</h2>
        </template>
                    ...
                    <div class="p-8">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-white/5">
                                <thead class="bg-gray-50 dark:bg-white/5">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Birthdate</th>
                                ...
                                <tbody class="bg-white dark:bg-transparent divide-y divide-gray-100 dark:divide-white/5">
                                    <tr v-for="author in authors.data" :key="author.id" class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap font-medium text-white">{{ author.name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-white">{{ author.email }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-gray-500 dark:text-gray-400">{{ author.birthdate }}</td>
                                        <td v-if="$page.props.auth.user.role === 'admin'" class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                            <Link :href="route('authors.edit', author.id)" class="inline-flex items-center px-3 py-1 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-lg hover:bg-indigo-100 transition-all">Edit</Link>
                                            <button @click="deleteAuthor(author.id)" class="inline-flex items-center px-3 py-1 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-100 transition-all">Delete</button>
                                        </td>
                                    </tr>
                                    <tr v-if="authors.data.length === 0">
                                        <td colspan="4" class="px-6 py-10 text-center text-gray-500">No authors found.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="mt-8 flex justify-center">
                             <nav class="relative z-0 inline-flex rounded-xl shadow-sm -space-x-px" aria-label="Pagination">
                                <template v-for="(link, k) in authors.links" :key="k">
